...
|------|-------|-------------|
| **Basic** (Free) | $0/mo | 500MB storage, basic profile |
| **Plus** | $7 – $10/mo | 5GB storage, analytics, YouTube sync |
| **Pro** | $15 – $19/mo | 50GB storage, "Verified Pro" badge, featured search |

**Feature Gating**:

- **Media Storage**: Video files (walkthroughs of job sites) are data-intensive. Free users are capped at 500MB; Plus users get 5GB; Pro users get 50GB.
- **"Verified Pro" Badge**: A trust signal derived from identity and license verification, crucial for winning independent bids. (Pro only)
- **Search Prominence**: Pro profiles appear in the "Featured" tier of recruiter searches.
- **Who Viewed Your Profile**: Detailed analytics on which general contractors (GCs) or homeowners are inspecting their work. (Plus and Pro)
...
| 4. Platform service fee (5% of remaining) | -$48.54 |
...
- **TradeFolio Positioning**: By offering a tiered model (Free / $7–10 Plus / $15–19 Pro), TradeFolio captures the "Prosumer" segment—individual ambitious workers and sole proprietors who cannot justify $300/mo for CRM software but need more than a free Instagram account. The Plus tier provides an accessible entry point for price-sensitive users, while Pro delivers the full verified professional experience.

# 3. Product Requirements Document (PRD)

## 3.1 Product Vision and Scope

**Vision**: To build the definitive digital ledger of the skilled trades, enabling every worker to turn their labor into a verified, portable asset. **Scope**: A mobile-first iOS and Android application (MVP) focusing on Portfolio Creation, Identity Verification, and Basic Networking. Phase 2 adds Payments and Job Search.
...
**Selection**: React Native (TypeScript). **Justification**:
...
**Challenge**: Users in basements or rural areas must be able to use the app without frustration. **Solution**: WatermelonDB (built on SQLite).
...
**Selection**: Node.js (NestJS framework) on AWS. **Justification**: NestJS provides an opinionated, angular-style architecture that enforces code quality and scalability, which is crucial as the startup grows from MVP to enterprise.
...
Video hosting is the single biggest technical risk for cost blowouts. Hosting raw 4K video uploaded by users is unsustainable. **Pipeline Design**:

- **Upload**: Mobile app requests a pre-signed URL from the API and uploads compressed video directly to AWS S3 (Ingest Bucket).
- **Processing**: An S3 event triggers an AWS MediaConvert job.
  - **Action**: Transcode raw video into HLS (HTTP Live Streaming) format with adaptive bitrates (360p, 720p, 1080p).
  - **Optimization**: Generate a WebP animated thumbnail for the feed preview.
- **Delivery**: Files are served via AWS CloudFront (CDN) to ensure low latency globally.
...
- **Encryption in Transit**: Enforce TLS 1.2+ for all API connections. Monitor Certificate Transparency (CT) logs to detect unauthorized certificates, rather than traditional certificate pinning (which is now discouraged due to maintenance burden and risk of app breakage during certificate rotation).
